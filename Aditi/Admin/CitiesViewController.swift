...
class CitiesViewController: UIViewController , UITableViewDataSource , UITableViewDelegate, City1CellDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let vc : Cities1ViewController = self.storyboard!.instantiateViewController()
        vc.parentCity = cities[indexPath.row]
    ...
    func deletePressed(cityItem: CityItem) {
        alertWithChoices(message: NSLocalizedString("areYouSure", comment: ""), yesBtnTitle: NSLocalizedString("yes", comment: ""), noBtnTitle: NSLocalizedString("no", comment: ""), yesaction: { [weak self] in
            ...
            ImpInfo.delCity(level: "cities_level1", cityitem: cityitem) { result , err in
    ...
    func listenCities(){
        ...
        ImpInfo.getInfo() {(data, err) in
            ...
            self.cities = data!.cities_level1
    ...
    func addCity(){
        ...
        let cityitem = CityItem.data(parent_city: "", name: city, city_code: cityCode)
        
        UIApplication.showLoader()
        ImpInfo.addCity(level: "cities_level1", cityitem: cityitem) { result , err in
            UIApplication.hideLoader()
            if let e = err {
                UIApplication.showError(message: e, delay: 1)
            }else{
...
extension CitiesViewController : DZNEmptyDataSetSource {
