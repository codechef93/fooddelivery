...
class Cities1ViewController: UIViewController , UITableViewDataSource , UITableViewDelegate, City1CellDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let vc : Cities2ViewController = self.storyboard!.instantiateViewController()
    ...
    func deletePressed(cityItem: CityItem) {
        alertWithChoices(message: NSLocalizedString("areYouSure", comment: ""), yesBtnTitle: NSLocalizedString("yes", comment: ""), noBtnTitle: NSLocalizedString("no", comment: ""), yesaction: { [weak self] in
            ...
            ImpInfo.delCity(level: "cities_level2", cityitem: cityitem) { result , err in
    ...
    func listenCities(){
        ...
        ImpInfo.getInfo() {(data, err) in
            ...
            self.cities = data!.cities_level2
    ...
    func addCity(){
        ...
        UIApplication.showLoader()
        ImpInfo.addCity(level: "cities_level2", cityitem: cityitem) { result , err in
            UIApplication.hideLoader()
            if let e = err {
                UIApplication.showError(message: e, delay: 1)
            }else{
...
extension Cities1ViewController : DZNEmptyDataSetSource {
