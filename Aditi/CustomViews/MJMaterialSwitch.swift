...
class MJMaterialSwitch: UIControl {
    ...
    #if Admin
        var isOn = User.shared?.online ?? false
    #else
        var isOn = User.shared?.notifications ?? false
    #endif
    ...
    func initialize() {
        ...
        #if Admin
            isOn = User.shared?.online ?? false
        #else
            isOn = User.shared?.notifications ?? false
        #endif
    ...
    func setStatus(){
        if !isOn {
            changeThumbStateOFFwithAnimation(sendAction: false)
            
        }else{
            changeThumbStateONwithAnimation(sendAction: false)
            self.switchThumb.backgroundColor = self.thumbOnTintColor
            self.track.backgroundColor = self.trackOnTintColor
        }
    }
    ...
    func changeThumbStateONwithAnimation(sendAction : Bool = true) {
    ...
    func changeThumbStateOFFwithAnimation(sendAction : Bool = true) {
...
//             Bouncing effect: Move thumb a bit, for better UX
    ...
    @objc func onTouchDown(btn: UIButton, withEvent event: UIEvent) {
        if !AppDelegate.hasInternet {return}
